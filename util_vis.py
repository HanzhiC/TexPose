...
def draw_samples(opt, var, draw_mode='all'):
    if draw_mode == 'object':
        ray_idx = var.ray_idx_obj
    elif draw_mode == 'background':
        ray_idx = var.ray_idx_bg
    elif draw_mode == 'all':
        ray_idx = var.ray_idx
    else:
    ...
    HW = opt.H * opt.W
    bg = torch.zeros(B, 1, opt.H, opt.W).contiguous()

    samples_cumsum = ray_idx + HW * torch.arange(B).cuda().unsqueeze(1)
    bg = bg.view(B * HW)
    bg[samples_cumsum] = 1.
    bg = bg.view(B, 1, opt.H, opt.W)
    return bg
