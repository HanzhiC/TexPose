...
class Dataset(torch.utils.data.Dataset):
    ...
    def preprocess_image(self, opt, image, is_tensor=False, aug=None):
        if aug is not None:
        ...
        if opt.data.image_size[0] is not None:
            if not is_tensor:
                ...
                image = torch_F.interpolate(image[None], (opt.H, opt.W), mode='nearest')[0]
        return image

    def preprocess_camera(self, opt, intr, aug=None):
        # center crop
